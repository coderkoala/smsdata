...
@section('content')
    <x-backend.card>
        ...
        <x-slot name="body">
            <form action="{{route('admin.edit-patch', isset($contact['ContactID'])?str_pad((string)$contact['ContactID'], 5, "0", STR_PAD_LEFT):'' )}}" method="POST">
                @csrf
                <div class="form-group row">
                  ...
                  <div class="col-8">
                    <select id="CategoryID" name="CategoryID" class="custom-select" required="required">
                      ...
                      <option {{null!==$contact && (int)$contact['CategoryID']===(int)$tuple['CategoryID']?'selected':''}} value="{{str_pad((string) $tuple['CategoryID'], 2, "0", STR_PAD_LEFT)}}">{{$tuple['CategoryName']}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
